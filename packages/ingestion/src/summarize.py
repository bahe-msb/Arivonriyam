...
def _build_prompt_ta(
    ...
    return "\n".join(filter(None, [
        ...
        "உங்கள் மாணவர்கள் 5 வயதுக்கு கீழ், எனவே மிகவும் எளிமையாகவும் வேடிக்கையாகவும் இருக்க வேண்டும்.",
        f"வகுப்பு: {class_name}  |  பாடம்: {subject}  |  தலைப்பு: {topic}",
        source_note,
        "",
        f"மாணவர் நிலை: வகுப்பு {level}. {grade_note}",
        "",
        "கற்பிக்கும் நடை விதிகள் (மிக முக்கியம்):",
        "• கதை சொல்வது போல பேசுங்கள், குறிப்புகளை ஓதுவது போல அல்ல.",
        "• emojis பயன்படுத்துங்கள்: 🍎 ஆப்பிள், ☀️ சூரியன், 🌳 மரம், 🐘 யானை போன்றவை.",
        "• உரையாடல் இருந்தால் இப்படி எழுதுங்கள்:",
        "  ஆசிரியர்: <ஆசிரியர் சொல்வது>",
        "  மாணவர்: <மாணவர் சொல்வது>",
        "• கணிதம் என்றால்: '🍎 + 🍎 = 2 ஆப்பிள்கள்!' போன்ற emoji காட்சிகளை பயன்படுத்துங்கள்.",
...
def _build_prompt_en(
    ...
    return "\n".join(filter(None, [
        ...
        "Your students are under 5 years old, so you must be extremely simple, fun, and engaging.",
        f"Class: {class_name}  |  Subject: {subject}  |  Topic: {topic}",
        source_note,
        "",
        f"Student level: Class {level}. {grade_note}",
        "",
        "TEACHING STYLE RULES (very important):",
        "• Talk like a teacher telling a story, not dictating notes.",
        "• Use fun emojis in your text: 🍎 for apple, ☀️ for sun, 🌳 for tree, 🐘 for elephant, etc.",
        "• If there is a conversation or dialogue in the text, format it as:",
        "  Teacher: <what teacher says>",
        "  Student: <what student says>",
        "  This makes it interactive and fun.",
        "• For math topics: use emoji visuals like '🍎 + 🍎 = 2 apples!' instead of plain numbers when possible.",
        ...
        *([f"• {note}" for note in multimodal_notes] if multimodal_notes else []),
        ...
        "",
        "content → 3-4 paragraphs. Each paragraph must include:",
        "           - The core idea in the simplest possible words",
        "           - An analogy: 'Think of it like…' or 'Imagine you are seeing…'",
        "           - A real-life example (home / classroom / nature) with emojis",
